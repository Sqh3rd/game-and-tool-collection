...
import { createSchemaModifier } from "../../../shared/utils/createSchemaModifier";
...
describe("createZodSchemasFromDBSchema", () => {
  ...
  const userRoleJunction = pgTable("userRoleJunction", {
  ...
  test("Base schema", () => {
    testBaseSchema(createSchemaModifier(dbSchema).create());
  });

  test("Base schema with relations", () => {
    testBaseSchema(
      createSchemaModifier(dbSchema).withRelations(relations).create(),
    );
  });

  test("Base schema with relations differs from plain base schema", () => {
    const baseSchema = createSchemaModifier(dbSchema).create();
    const baseSchemaWithRelations = createSchemaModifier(dbSchema)
  ...
  test("Base schema with modifications", () => {
    const baseSchema = createSchemaModifier(dbSchema)
  ...
  test("Schema properly extends withSelected", () => {
    ...
    const actual = baseSchema.user.selectWithRelations({ roles: true });
